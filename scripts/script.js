...
function addComments(i) {                                       //dient dazu Kommmentare zu einem Buch hinzuzufügen
    let commentInputRef = document.getElementById(`send_comments${i}`); // hier wird die Variable "commentInpuRef" erstellt welche auf das Inputfeld mit der id send_commments zugreift
    let commentInput = commentInputRef.value;                   // hier  wwird die var commentInput erstellt und mit dem commentInpuRef.value befüllt. Das .value gibt an das die eingabe vom Inputfeld genommen werden soll
    ...
    commentInputRef.value = "";                                 // nach der Eingabe des Kommentares wird das Inputfeld hier wieder geleert
...
function likesCounter(i) {                                          // diese Function verwaltet die Likes auf der Website mit dem Pararmeter i der für ein bestimmtes Buch steht
    ...
    let likesCount = document.getElementById(`likes_count${i}`);    // var likesCount wird erstellt, sucht die id likesCount${i} und setzt diese dem inhalt der id gleich
    ...
    if (unlikedHeart.classList.contains("d_none")) {                // hier wird der aktuelle zustand der id durch contains geprüft -> in dem Fall ob die klasse d_none enthalten ist, also das gefällt mir nicht icon nicht Sichtbar ist
        ...
        likedHeart.classList.remove("d_none");                      // dann wird hiermit dem Elemment likedHeart die klasse d_none entfernt, also es sichtbar gemacht
...
function checkLikedContent() {                                      // diese Funktion wird aufgerufen um den aktuellen like-Status der Bücher zu prüfen
    ...
    for(let i = 0; i < books.length; i++) {                             // die schleife läuft durch das Array in dem alle Bücher enthalten sind wobei der index nach jedem durchlauf um eins erhöht wird bis die länge des Arrays kleiner als i ist
        ...
        }else {                                                         // wenn der liked status des Buches mit dem index i false ist wird dieser else Block ausgeführt
...
function saveToLocalStorage() {                                     // diese function solll Daten im localStorage speichern und wird in der function addComments(i) aufgerufen um die Kommentare zu speichern
